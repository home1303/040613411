...
<?php 
  ...
  if (isset($_POST['update'])) {
      ...
      $sql = $conn->prepare("UPDATE oder_item_hosting SET oder_status = :sele_status WHERE oder_id = :oder_id");
      $sql->bindParam(":oder_id", $oder_id);
      ...
      if ($sql) {
          ...
          header("refresh:2; url=admin_manage_hosting_1.php");
      } else {
          $_SESSION['error'] = "Data has not been updated succesfully";
          header("location: admin_manage_hosting_1.php");
      }
}
?>
        ...
        <h1 class="text-uppercase text-center mb-3">แก้ไขข้อมูล ✍️</h1>
          ...
          <?php 
                if (isset($_GET['update_id'])) {
                    ...
                    $stmt = $conn->query("SELECT * FROM oder_item_hosting WHERE oder_id = '$update_id'");
                    $stmt->execute();
                    $data = $stmt->fetch();
                }
            ?>
            ...
            <hr>
              <div class="mb-3">
                    ...
                    <input type="text" readonly value="<?= $data['oder_id']; ?>" required class="form-control" name="oder_id">
                </div>
                <div class="mb-3">
