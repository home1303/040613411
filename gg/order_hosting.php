...
<?php 
    ...
    if (isset($_GET['item'])) {
        $item = $_GET['item'];
        $stmt = $conn->query("SELECT * FROM package_plan_hosting JOIN package_plan_domain WHERE pack_hosting_id = $item");
        $stmt->execute();
        $data = $stmt->fetch();
    }
?>
...
<?php 
    ...
    if (isset($_POST['submit'])) {
      ...
      $oder_package_id = $data['pack_hosting_id'];
      $oder_package_type = $data['hosting_pack_name'];
      $oder_domain = $_POST['oder_domain'];
      ...
      $oder_name_hosting = $_POST['oder_name_hosting'];
      $oder_password_hosting = $_POST['oder_password_hosting'];
      ...
      if(empty($oder_name_hosting)){
        echo "<script>
        $(document).ready(function() {
            Swal.fire({
                title: 'มีบางอย่างผิดพลาด!',
                text: 'กรุณาป้อนชื่อโฮส!',
                icon: 'error',
                timer: 3000,
      ...
      elseif(empty($oder_password_hosting)){
        echo "<script>
        $(document).ready(function() {
            Swal.fire({
                title: 'มีบางอย่างผิดพลาด!',
                text: 'กรุณาป้อนรหัสผ่านโฮส!',
                icon: 'error',
                timer: 3000,
                showConfirmButton: false
                });
        ...
        if($row['amount'] >= $result_explode[0]){
          ...
          $sql = $conn->prepare("INSERT INTO oder_item_hosting(oder_package_id, oder_package_type, oder_domain, oder_price, oder_time_rent, oder_time_rent_date_type, oder_name_hosting, oder_password_hosting, oder_email, oder_time_start, oder_time_end, oder_status,oder_status_price)
          VALUES(:oder_package_id, :oder_package_type, :oder_domain, :oder_price, :oder_time_rent, :oder_time_rent_date_type, :oder_name_hosting, :oder_password_hosting, :oder_email, :oder_time_start, :oder_time_end, :oder_status , :oder_status_price)");
          $sql->bindParam(":oder_package_id", $oder_package_id);
          $sql->bindParam(":oder_package_type", $oder_package_type);
          $sql->bindParam(":oder_domain", $oder_domain);
          ...
          $sql->bindParam(":oder_name_hosting", $oder_name_hosting);
          $sql->bindParam(":oder_password_hosting", $oder_password_hosting);
                ...
                <div class="col">
                  <div class="card-deck">
                    <div class="card text-center" style="height: 410px;">
                      ...
                      <i class='bx bxs-data h1' ></i>
                        <h5 class="card-title fw-bold"><?= $data['hosting_pack_name']; ?></h5>
                        <div class="p-4">
                          <p class="card-text h5" style='color:rgba(165, 165, 165, 0.842)'><i class='bx bx-globe' style="width:25px"></i> <?= $data['hosting_pack_domains']; ?> Domain </p>
                          <p class="card-text h5" style='color:rgba(165, 165, 165, 0.842)'><i class='bx bxs-hdd' style="width:25px"></i> <?= $data['hosting_pack_space']; ?> GB of Space</p>
                          <p class="card-text h5" style='color:rgba(165, 165, 165, 0.842)'><i class='bx bx-transfer-alt' style="width:25px"></i> <?= $data['hosting_pack_transfer']; ?> GB Monthly transfer</p>
                          <p class="card-text h5" style='color:rgba(165, 165, 165, 0.842)'>  <?= $data['hosting_pack_database']; ?> Database of MySQL</p>
                        </div>
                        <div class="p-2 mb-1">
                          <p class="card-text h4 fw-bold"><?= $data['hosting_pack_price_y']; ?> ฿/yearly</p>
                          <p class="card-text text-muted"><?= $data['hosting_pack_price_y']; ?>฿/yearly</p>
                        </div>
              ...
              <form action="" method="post" enctype="multipart/form-data">
                <div class="col">
                  <div class="card-deck">
                    <div class="card text-center">
                      <div class="card-body shadow-sm">
                        <h5 class="card-title fw-bold"><span class="badge bg-primary rounded-pill" style="width: 130px; height: auto;" value="<?= $data['hosting_pack_name']; ?>"><?= $data['hosting_pack_name']; ?> </span>
                        <span class="badge bg-primary rounded-pill" style="width: 75px; height: auto;" value="<?= $data['pack_hosting_id']; ?>">#<?= $data['pack_hosting_id']; ?> </span></h5>

                          <div class="form-floating m-3">
                            <select class="form-select" id="floatingSelect" aria-label="Floating label select example" name="oder_domain">

                            <?php
                              $result1 = $conn->query("SELECT * FROM package_plan_domain");
                              if ($result1 !== false) {
                                while($item1 = $result1->fetch()) {

                            ?>

                              <option value="<?= $item1['pack_domain_name']; ?>"><?= $item1['pack_domain_name']; ?></option>

                              <?php } 
                                } ?>

                            </select>
                            <label for="floatingSelect">เลือก Domain</label>
                          </div>

                          <div class="form-floating m-3">
                            <select class="form-select" id="floatingSelect" aria-label="Floating label select example" name="select_rent">
                              <option value="<?= $data['hosting_pack_price_y']; ?>|1|Yearly"><?= $data['hosting_pack_price_y']; ?>฿ / 1 ปี</option>
                            </select>
                            <label for="floatingSelect">จำนวนในการเช่า</label>
                          </div>

                          <div class="form-floating m-3">
                            <input type="text" class="form-control" id="floatingInput" name="oder_name_hosting" placeholder="text">
                            <label for="floatingInput">ชื่อโฮส</label>
                          </div>

                          <div class="form-floating m-3">
                            <input type="text" class="form-control" id="floatingInput" name="oder_password_hosting" placeholder="text">
                            <label for="floatingInput">รหัสผ่านโฮส</label>
                          </div>
                    
                          <button type="submit" id="submit" name="submit" class="btn btn-success">Submit</button>
                        <a href="Hosting.php" class="btn btn-danger">ยกเลิก</a>
                      </div>
                    </div>
                  </div>
                </div>
