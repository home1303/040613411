...
<?php 
    ...
    if (isset($_GET['item'])) {
      $item = $_GET['item'];
      $stmt = $conn->query("SELECT * FROM oder_item WHERE oder_ip = '$item'");
      $stmt->execute();
      $data = $stmt->fetch();
  ...
  <?php
      if(isset($_POST['update_info'])){
        ...
        $id1 = $_GET['id1'];
        $newRam = $_POST['new_ram'];
        $newCore = $_POST['new_core'];
        $newStorage = $_POST['new_storage'];

        $sql = $conn->prepare("UPDATE package_plan_vps SET vps_pack_memory = :new_ram, vps_pack_core = :new_core, vps_pack_storage = :new_storage WHERE pack_vps_id = '$id1'");
        $sql->bindParam(":new_ram", $newRam);
        $sql->bindParam(":new_core", $newCore);
        $sql->bindParam(":new_storage", $newStorage);
        $sql->execute();
        ...
        if($sql){
          ...
          header("refresh:2; url=server_manage.php");
        }
        else{
          echo "<script>
          $(document).ready(function() {
...
<?php
      if(isset($_POST['update_ip'])){
        ...
        $sql = $conn->prepare("UPDATE oder_item SET oder_ip = :newIP WHERE oder_ip = :oldIP");
        $sql->bindParam(":oldIP", $oldIP);
        $sql->bindParam(":newIP", $newIP);
        $sql->execute();
...
<?php 
    if(isset($_POST['update_password_server'])){
      ...
      $dataPassword = $data['oder_password_server'];
      ...
      elseif(empty($newPassword)){
        ...
        if($oldPassword == $data['oder_password_server']){

          $oldIP = $data['oder_ip'];

          $sql = $conn->prepare("UPDATE oder_item SET oder_password_server = :newPassword WHERE oder_ip = :oldIP");
          $sql->bindParam(":oldIP", $oldIP);
  ...
  <?php
    if(isset($_POST['add_time'])){
      ...
      if ($result_explode[2] == 'Daily') {
        $add_time_rent = date('Y-m-d H:i:s', strtotime($new_time_end. ' + 1 days')); //DATE +1 d
        $oder_time_end = $add_time_rent;
      }
      elseif($result_explode[2] == 'Monthly'){
        $add_time_rent = date('Y-m-d H:i:s', strtotime($new_time_end. ' + 1 months')); //DATE +1 m
        $oder_time_end = $add_time_rent;
      }
      ...
      if($row['amount'] >= $result_explode[0]){
          ...
          $oldIP = $data['oder_ip'];
          $sql = $conn->prepare("UPDATE oder_item SET oder_time_end = :oder_time_end WHERE oder_ip = :oldIP");
          $sql->bindParam(":oldIP", $oldIP);
  ...
  <section class="home-section">
    <div class="home-content">
      ...
      <span class="text" style='color:#ffffff' >VPS MANAGER</span>
    </div>
  ...
  <div class="p-4"></div>
    ...
    <div class="container">
      ...
      ?>
      <div class="card text-center" style="height: 760px;" >
        <div class="card-body shadow-lg">
          <h1 class="text-uppercase text-center mb-3">VPS SERVER MANAGEMRENT ⚙️<p class="fs-5">จัดการเครื่องเซิร์ฟของคุณ </p></h1>
                  ...
                  <div class="card-deck">
                    <div class="card text-center" style="height: 760px;">
                      <div class="card-body shadow-sm">
                        ...
                        <div class="p-4">
                          <p class="card-text h5"><span class="badge bg-primary rounded-pill" style="width: 130px; height: auto;">IP ADDRESS</span><span class="badge" style="color: black; width: 130px; height: auto;"><?php echo $data['oder_ip'] ?></span></p>
                          <p class="card-text h5"><span class="badge bg-primary rounded-pill" style="width: 130px; height: auto;">NAME SERVER</span><span class="badge" style="color: black; width: 130px; height: auto;"><?php echo $data['oder_name_server'] ?></span></p>
                          <p class="card-text h5"><span class="badge bg-primary rounded-pill" style="width: 130px; height: auto;">PASSWORD</span><span class="badge" style="color: black; width: 130px; height: auto;"><?php echo $data['oder_password_server'] ?></span></p>
                        ...
                        <p class="card-text h6" style='color:rgba(165, 165, 165, 0.842)'>** กรณีเปลี่ยน IP เสียครั้งละ 200 บาท **</p>
                        <button type="submit" id="update_ip" name="update_ip" class="btn btn-success">เปลี่ยน IP ADDRESS</button>
                      </div>
              ...
              <div class="col">
                ...
                <div class="col">
                  ...
                  <form action="" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-floating m-3">
                          <select class="form-select" id="floatingSelect" name="select_rent" aria-label="Floating label select example">
                            ...
                            <?php
                              ...
                              $result19 = $conn->query("SELECT * FROM package_plan_vps WHERE pack_vps_id = $pack_display");
                              $item19 = $result19->fetch();

                            ?>
                            <option value="<?php echo $item19['vps_pack_price_d'] ?>|1|Daily"><?php echo $item19['vps_pack_price_d'] ?>฿ / 1 วัน</option>
                            <option value="<?php echo $item19['vps_pack_price'] ?>|1|Monthly"><?php echo $item19['vps_pack_price'] ?>฿ / 1 เดือน</option>
                            <option value="<?php echo $item19['vps_pack_price_y'] ?>|1|Yearly"><?php echo $item19['vps_pack_price_y'] ?>฿ / 1 ปี</option>
                          </select>
                          ...
                          <div class="form-floating m-3">
                            ...
                            <label for="floatingInput">รหัสผ่านเครื่องเซิร์ฟเวอร์เดิม</label>
                          </div>
                          <div class="form-floating m-3">
                            <input type="text" class="form-control" name="newPassword" id="floatingInput" placeholder="text">
                            <label for="floatingInput">รหัสผ่านเครื่องเซิร์ฟเวอร์ใหม่</label>
                          </div>
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                    <div class="form-floating m-3">
                            <input type="number" class="form-control" name="new_ram" id="floatingInput" placeholder="text" min="0" max="64" required>
                            <label for="floatingInput">จำนวนที่ต้องการเพิ่ม RAM</label>
                          </div>

                          <div class="form-floating m-3">
                            <input type="number" class="form-control" name="new_core" id="floatingInput" placeholder="text" min="2" max="12" required>
                            <label for="floatingInput">จำนวนที่ต้องการเพิ่ม CORE</label>
                          </div>

                          <div class="form-floating m-3">
                            <input type="number" class="form-control" name="new_storage" id="floatingInput" placeholder="text" min="1" max="999" required>
                            <label for="floatingInput">จำนวนที่ต้องการเพิ่ม STORAGE</label>
                          </div>

                        <button type="submit" id="update_info" name="update_info" class="btn btn-success">ยืนยัน</button>
                        <a href="server_manage.php" class="btn btn-danger">ยกเลิก</a>
                      </div>
                    </div>
                    </form>
