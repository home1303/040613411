...
<?php 
  ...
  if (isset($_POST['update'])) {
      $oder_ip = $_POST['oder_ip'];
      $sele_status = $_POST['sele_status'];

      $sql = $conn->prepare("UPDATE oder_item SET oder_status = :sele_status WHERE oder_ip = :oder_ip");
      $sql->bindParam(":oder_ip", $oder_ip);
      $sql->bindParam(":sele_status", $sele_status);
      $sql->execute();
      ...
      if ($sql) {
          ...
          header("location: admin_manage_oder.php");
      }
}
?>
        ...
        <h1 class="text-uppercase text-center mb-3">แก้ไขข้อมูล ✍️</h1>
          ...
          <?php 
                if (isset($_GET['update_id'])) {
                    ...
                    $stmt = $conn->query("SELECT * FROM oder_item WHERE oder_ip = '$update_id'");
                    $stmt->execute();
                    $data = $stmt->fetch();
                }
            ?>
            ...
            <hr>
              <div class="mb-3">
                    <label for="id" class="d-flex justify-content-start col-form-label">IP ADDRESS : </label>
                    <input type="text" readonly value="<?= $data['oder_ip']; ?>" required class="form-control" name="oder_ip">
                </div>
                <div class="mb-3">
                    <label for="firstname" class="d-flex justify-content-start col-form-label">STATUS : </label>
                    <select class="form-select" name="sele_status" aria-label="Default select example">
                      <option selected><?= $data['oder_status']; ?></option>
                      <?php
                        $result = $conn->query("SELECT * FROM status");
                        if ($result !== false) {
                        while($item = $result->fetch()) {
                      ?>
                      <option value="<?php echo $item['status_type'] ?>"><?php echo $item['status_type'] ?></option>
                      <?php } 
                        } ?>
                    </select>
                </div>
