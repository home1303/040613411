...
<?php 
    ...
    if (isset($_GET['item'])) {
        $item = $_GET['item'];
        $stmt = $conn->query("SELECT * FROM package_plan_ddc WHERE pack_ddc_id = $item");
        $stmt->execute();
        $data = $stmt->fetch();
    }
?>
...
<?php 
    ...
    if (isset($_POST['submit'])) {
      ...
      $oder_package_id = $data['pack_ddc_id'];
      $oder_package_type = $data['ddc_pack_name'];
      ...
      elseif(empty($oder_password_server)){
        ...
        if($row['amount'] >= $result_explode[0]){
          ...
          $sql = $conn->prepare("INSERT INTO oder_item(oder_package_id, oder_package_type, oder_os, oder_price, oder_time_rent, 
          oder_time_rent_date_type, oder_name_server, oder_password_server, oder_email, oder_time_start, oder_time_end, 
          oder_status, oder_ip, oder_status_price)
          VALUES(:oder_package_id, :oder_package_type, :oder_os, :oder_price, :oder_time_rent, :oder_time_rent_date_type, 
          :oder_name_server, :oder_password_server, :oder_email, :oder_time_start, :oder_time_end, :oder_status, :oder_ip, :oder_status_price)");
...
?>
  ...
  <section class="home-section">
    <div class="home-content">
      ...
      <span class="text" style='color:#ffffff' >Dedicated Server</span>
    </div>
          ...
          <div class="container">
            <div class="row row-cols-2">
              <div class="col">
                ...
                <div class="col">
                  <div class="card-deck">
                    <div class="card text-center" style="height: 440px;">
                      <div class="card-body shadow-sm">
                        <i class='bx bxl-windows h1'></i>
                        <i class='bx bxl-tux h1' ></i>
                        <?php 
                          if($data['ddc_cpu_type'] == 'AMD'){
                            $cpu_color = 'red';
                          }
                          elseif($data['ddc_cpu_type'] == 'INTEL'){
                            $cpu_color = 'blue';
                          }
                        ?>
                        <h5 class="card-title fw-bold" style='color:<?php echo $cpu_color ?>'><?= $data['ddc_cpu_type']; ?></h5><h5><?= $data['ddc_dis_name']; ?></h5>
                        <div class="p-4">
                          <p class="card-text h5" style='color:rgba(165, 165, 165, 0.842)'><i class="fa-solid fa-microchip"></i> <?= $data['ddc_pack_core']; ?> vCPU</p>
                          <p class="card-text h5" style='color:rgba(165, 165, 165, 0.842)'><i class="fa-solid fa-memory"></i> RAM <?= $data['ddc_pack_memory']; ?> GB</p>
                          <p class="card-text h5" style='color:rgba(165, 165, 165, 0.842)'><i class="fa-solid fa-hard-drive"></i> SSD <?= $data['ddc_pack_storage']; ?> GB</p>
                        ...
                        <div class="p-2 mb-1">
                          <p class="card-text h4 fw-bold"><?= number_format( $data['ddc_pack_price'] , 2 ) ?> ฿</p>
                          <p class="card-text text-muted"><?= number_format( $data['ddc_pack_price_y'] , 2 ) ?>฿/yearly</p>
                        </div>
              ...
              <form action="" method="post" enctype="multipart/form-data">
                <div class="col">
                  <div class="card-deck">
                    <div class="card text-center" style="height: 440px;">
                      <div class="card-body shadow-sm">
                        <h5 class="card-title fw-bold"><span class="badge bg-primary rounded-pill" style="width: 130px; height: auto;" value="<?= $data['ddc_pack_name']; ?>"><?= $data['ddc_pack_name']; ?> </span>
                        <span class="badge bg-primary rounded-pill" style="width: 75px; height: auto;" value="<?= $data['pack_ddc_id']; ?>">#<?= $data['pack_ddc_id']; ?> </span></h5>
                          ...
                          <div class="form-floating m-3">
                            <select class="form-select" id="floatingSelect" aria-label="Floating label select example" name="select_rent">
                              <option value="<?= $data['ddc_pack_price']; ?>|1|Monthly"><?= number_format( $data['ddc_pack_price'] , 2 ) ?>฿ / 1 เดือน</option>
                              <option value="<?= $data['ddc_pack_price_y']; ?>|1|Yearly"><?= number_format( $data['ddc_pack_price_y'] , 2 ) ?>฿ / 1 ปี</option>
                            </select>
                        ...
                        <a href="oder_dedicated.php" class="btn btn-danger">ยกเลิก</a>
                      </div>
                    </div>
                  </div>
                </div>
